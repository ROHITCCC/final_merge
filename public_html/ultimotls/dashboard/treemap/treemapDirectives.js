    ...
    function createZoomTree(treeData, element, flag, scope, resizedWin){
        ...
        if(jsonRaw !== undefined)
        {
            ...
            function sendAudit(parent, name){       //sends audits directly instead of through controller function
                ...
                var interfaceQuery = '{"application":"'+name+'","interface1":"'+parent+'","envid":"'+scope.env.dbName+'","timestamp":{"$gte":{"$date":"'+scope.fromDate+'"},"$lt":{"$date":"'+scope.toDate+'"}},"$and":[{"severity":{"$ne":"null"}},{"severity":{"$exists":"true","$ne":""}}]}';
